# Testing & build recipe — TurboQuant SYCL fork
...
### Configure (RECOMMENDED — official perf build for Arc B580 / Xe2)
Measured 2026-07-09: these flags roughly double prefill (pp512 +102.8%, 2.03x) and keep decode flat
(-3.3%, within noise), with the golden `test-sycl-turbo` gate GREEN (22/22) and coherent output. See
...
```
- `GGML_SYCL_F16=ON` — enables the FP16 matmul path on Xe2's XMX units (the source of the prefill gain).
- `GGML_SYCL_DEVICE_ARCH=bmg-g21` — AOT compilation via `spir64_gen`, eliminating the JIT warmup at
  runtime. **Careful: use the hyphen form `bmg-g21`** (what ocloc 2026.0 expects for this CMake path);
  the underscore form `bmg_g21` is the `-fsycl-targets=intel_gpu_bmg_g21` selector, a different
  mechanism, and does NOT work through this CMake path. Trade-off: the AOT link is very slow (~1h+)
  and `ggml-sycl.dll` ends up ~247 MB.
- `GGML_SYCL_DNN=ON` — enables oneDNN.

> **Xe2 caveat:** the F16/AOT flags enable more aggressive numeric paths. They are safe **on this
> GPU** (golden green + coherent output on the B580), but **re-running the golden gate is mandatory**
> on any other arch/toolchain before trusting them there. If the golden gate fails, or the slow AOT
> build is inconvenient, the fallback is F16-only without AOT (fast link, pays JIT cost on the first
> run) or the baseline configuration below.

### Configure (baseline / fallback — no perf flags)
```
...
2. **Turbo KV e2e COHERENCE gate (MANDATORY — needs the reference model + a GPU)** —
   ...
   **self-hosted Arc box**. **Last run: PASS on B580** (3 symmetric + 6 asymmetric all coherent).
   Full result matrix + samples in `docs/en/benchmarks.md` ("e2e COHERENCE matrix").
