...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ggml_init_params params = {
                // TURBO: +3 for turbo rotation matrices (turbo_rotation + turbo_rotation_inv + turbo_innerq_scale_inv)
    ...
    if (other) {
        // shared-cell target: inherit head dims and rotation policy from the source cache
        ...
        //
        // LLAMA_ATTN_ROT_DISABLE preserved as a hard lock-out (=1 forces rotation
        // off and blocks the overrides below).
        ...
        // Per-side K override. Guards below still apply: rotation only takes effect
        // on quantized types with head_dim % 64 == 0 (master's #21038 requirements).
