...
// TURBO BEGIN — keep GGML_SYCL_FA_ALL_QUANTS OFF (fork's validated FA config).
// Turbo (TURBO3_0) has extern-template FA-vec decls for every (type_K,type_V) pair at all D,
// but only 3 explicit instances exist (tq3-tq3, q8_0-tq3, tq3-q8_0 at D=64/128 — turbo GRF-spills
// at D>=256 on Intel GPUs, see FATTN_VEC_CASES_TURBO_D in fattn.cpp). Upstream's default of
// #define GGML_SYCL_FA_ALL_QUANTS activates the ALL-D branch that odr-uses turbo x standard and
// turbo @ D256/512 combos -> LNK2019 unresolved. Undefined here restores the fork's curated
// #else FA path (F16/Q4_0/Q8_0 standard + curated turbo). Re-enabling ALL_QUANTS requires
// generating the full turbo instance matrix first.
// #define GGML_SYCL_FA_ALL_QUANTS //upstream default; disabled for turbo (see above)
// TURBO END
