...
static bool ggml_sycl_tq_convert_q8() {
    static int val = -1;
    if (val == -1) {
        val = ggml_sycl_get_env("GGML_TQ_NATIVE", 0) == 1 ? 0 : 1; // default ON, GGML_TQ_NATIVE=1 disables
    }
    return val == 1;
}
...
static bool can_use_dequantize_mul_mat_vec(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    return ggml_sycl_supports_dmmv(src0->type) && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32 &&
           ...
           // TURBO BEGIN — turbo KV types must not go through dmmv (they use the fused turbo path)
        ...
        case GGML_OP_MUL_MAT_ID:
            {
                ...
                // TURBO BEGIN — TQ weight quant types are supported for MUL_MAT / MUL_MAT_ID
        ...
        case GGML_OP_SET_ROWS:
            {
                ...
                auto res = ((op->type == GGML_TYPE_F32 || op->type == GGML_TYPE_F16 || op->type == GGML_TYPE_BF16 ||
                         ...
                         /* TURBO BEGIN */ op->type == GGML_TYPE_TURBO2_0 || op->type == GGML_TYPE_TURBO3_0 || op->type == GGML_TYPE_TURBO4_0 ||
                         op->type == GGML_TYPE_TQ3_1S || op->type == GGML_TYPE_TQ4_1S /* TURBO END */) &&
                        op->src[0]->type == GGML_TYPE_F32 &&
