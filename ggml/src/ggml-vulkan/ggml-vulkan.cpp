        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                ...
                // NOTE: turbo3_0 flash-attention is not included in fa_kv_ok above.
                // The scalar/coopmat1 FA shaders no longer compile a turbo3_0-specialized
                // variant (see vulkan-shaders-gen.cpp/flash_attn_base.glsl), so claiming
                // support here without the matching dequant path would silently produce
                // zeros. CM2 turbo3 decode (dequant_funcs_cm2.glsl) is unaffected but is
                // gated by the same fa_kv_ok() check.
