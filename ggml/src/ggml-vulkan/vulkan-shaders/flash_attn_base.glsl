...
// TURBO3_0's dequantize4() still lives behind the old DATA_A_TURBO3_0
// compile-time path (see below); it is not yet wired into the FaType
// runtime dispatch used by the other quants here.
...
// NOTE: turbo3_0 flash-attention (scalar + coopmat1) is not yet re-integrated
// with upstream's FaType runtime dispatch. The old per-shader-variant
// DATA_A_TURBO3_0 dequantize4() (with dedicated K_T3/V_T3 bindings) was
// dropped here because vulkan-shaders-gen.cpp no longer compiles a
// turbo3_0-specialized flash_attn.comp / flash_attn_cm1.comp variant
// (upstream now compiles one generic shader per fp16/f16acc combo and
// dispatches on FaTypeK/FaTypeV at runtime). Restoring turbo3 FA requires
// adding a TURBO3_0 case (aliased K/V buffer view + dequant body) to the
// new flash_attn_dequant.glsl, plus host-side FaTypeK/FaTypeV wiring in
// ggml-vulkan.cpp. CM2 (dequant_funcs_cm2.glsl) turbo3 decode is unaffected.
