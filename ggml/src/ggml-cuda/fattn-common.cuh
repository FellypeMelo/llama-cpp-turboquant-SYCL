...
void launch_fattn(
...
#ifdef GGML_USE_HIP
    // HIP/ROCm: bypass the memory pool / persistent tensor-buffer padding for f16 temp
    // buffers. Both retain peak-sized allocations permanently, so for quantized KV
    // dequant the f16 temp buffer keeps consuming more VRAM than the quantized KV
    // compression saves, causing OOM. Raw alloc+free (freed when this scope exits)
    // ensures the memory is released right after the kernel completes instead of
    // being retained at the historical peak size.
    struct hip_f16_alloc {
        ...
        uintptr_t alloc(size_t nelements) {
            CUDA_CHECK(cudaMalloc(&ptr, nelements * sizeof(half)));
            return (uintptr_t) ptr;
        }
    };
    hip_f16_alloc K_f16_raw(main_stream);
    hip_f16_alloc V_f16_raw(main_stream);

    f16_extra = {};
    if (need_f16_K && K->type != GGML_TYPE_F16) {
        f16_extra.K = K_f16_raw.alloc(ggml_nelements(K));
    }
    if (need_f16_V && V->type != GGML_TYPE_F16) {
        if (V_is_K_view) {
            f16_extra.V = f16_extra.K;
        } else {
            f16_extra.V = V_f16_raw.alloc(ggml_nelements(V));
        }
    }
#else
    f16_extra = ggml_cuda_flash_attn_ext_get_f16_extra_data(KQV, need_f16_K, need_f16_V);
#endif // GGML_USE_HIP
